...
def start_inst(start_font1, start_font1_rect):
    canvas.fill(BLACK)
    canvas.blit(start_font1, start_font1_rect)
    start_inst1 = small_font.render(
        "--> Do not cross the edges, else you will be dead", True, BLUE)
    start_inst2 = small_font.render(
        "--> Keep eating the red apples", True, BLUE)
    start_inst3 = small_font.render(
        "--> Do not cross over yourself", True, BLUE)
...
def gameover():
    ...
    font_gameover2_rect.center = (
        WINDOW_WIDTH / 2 + 150, WINDOW_HEIGHT / 2 + 20)
    font_gameover3_rect.center = (
    ...
    while True:
        ...
        while True:
            ...
            snake_head_rect = pygame.Rect(
                LEAD_X, LEAD_Y, SNAKE_WIDTH, SNAKE_WIDTH)
            ...
            if snake_head_rect.colliderect(apple_rect):
                ...
                score = small_font.render(
                    "Score:" + str(snakelength - 3), True, YELLOW)

            canvas.blit(score, (20, 10))
            pygame.draw.line(canvas, GREEN, (0, TOP_WIDTH),
                             (WINDOW_WIDTH, TOP_WIDTH))
            pygame.draw.line(canvas, YELLOW, (WINDOW_WIDTH -
                                              60, 0), (WINDOW_WIDTH - 60, TOP_WIDTH))
            pygame.draw.rect(
                canvas, YELLOW, (WINDOW_WIDTH - 60, 0, 60, TOP_WIDTH))
