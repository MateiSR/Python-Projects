...
def Database():
    global conn, cursor
    conn = sqlite3.connect("snakedb.db")
    cursor = conn.cursor()
    cursor.execute(
...
def Login(event=None):
    ...
    if USERNAME.get() == "" or PASSWORD.get() == "":
        ...
        if cursor.fetchone() is not None:
            print("Succesfully logged in! Starting Snake Game!")
            root.withdraw()
            system(
                "C:/Python38/python.exe d:/GitHub/Python-Projects/snake_game/snake_game.py")
            USERNAME.set("")
            PASSWORD.set("")
...
# ==============================LABELS=========================================
lbl_title = Label(Top, text="Snake Game: Login", font=('arial', 15))
