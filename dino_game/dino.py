...
jump_sound = pygame.mixer.Sound(os.path.join(cpath, 'resources/jump.wav'))
die_sound = pygame.mixer.Sound(os.path.join(cpath, 'resources/die.wav'))
checkPoint_sound = pygame.mixer.Sound(
    os.path.join(cpath, 'resources/checkPoint.wav'))
...
class Dino():
    def __init__(self, sx=-1, sy=-1):
        self.imgs, self.rect = load_sprite_sheet(
            os.path.join(cpath, 'resources/dino.png'), 5, 1, sx, sy, -1)
        self.imgs1, self.rect1 = load_sprite_sheet(
            os.path.join(cpath, 'resources/dino_ducking.png'), 2, 1, 59, sy, -1)
...
class Cactus(pygame.sprite.Sprite):
    def __init__(self, speed=5, sx=-1, sy=-1):
        ...
        self.imgs, self.rect = load_sprite_sheet(
            os.path.join(cpath, 'resourcescactus-small.png'), 3, 1, sx, sy, -1)
        self.rect.bottom = int(0.98 * height_screen)
...
class birds(pygame.sprite.Sprite):
    def __init__(self, speed=5, sx=-1, sy=-1):
        ...
        self.birds_height = [height_screen * 0.82,
                             height_screen * 0.75, height_screen * 0.60]
...
class Ground():
    def __init__(self, speed=-5):
        self.image, self.rect = load_image(os.path.join(
            cpath, 'resources/ground.png'), -1, -1, -1)
        self.image1, self.rect1 = load_image(
            os.path.join(cpath, 'resources/ground.png'), -1, -1, -1)
...
def introduction_screen():
    ...
    t_ground, t_ground_rect = load_sprite_sheet(
        os.path.join(cpath, 'resources/ground.png'), 15, 1, -1, -1, -1)
    t_ground_rect.left = width_screen / 20
    t_ground_rect.bottom = height_screen

    logo, l_rect = load_image(os.path.join(
        cpath, 'resources/logo.png'), 300, 140, -1)
...
def gameplay():
    ...
    rbtn_image, rbtn_rect = load_image(os.path.join(
        cpath, 'resources/replay_button.png'), 35, 31, -1)
    gmo_image, gmo_rect = load_image(os.path.join(
        cpath, 'resources/game_over.png'), 190, 11, -1)

    t_images, t_rect = load_sprite_sheet(
        'numbers.png', 12, 1, 11, int(11*6/5), -1)
    ...
    while not g_exit:
        ...
        while not g_Over:
                    ...
                    if event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_SPACE:
                            if gamer_Dino.rect.bottom == int(0.98 * height_screen):
                                ...
                                if pygame.mixer.get_init() != None:
                                    jump_sound.play()
                                gamer_Dino.movement[1] = - \
                                    1*gamer_Dino.jumpSpeed
            ...
            if len(skyClouds) < 5 and random.randrange(0, 300) == 10:
                Cloud(width_screen, random.randrange(
                    height_screen / 5, height_screen / 2))
